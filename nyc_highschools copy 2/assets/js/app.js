  ...
  onEachFeature: function (feature, layer) {
    if (feature.properties) {
      var content =
        ...
        '<tr><td colspan="2" class ="chartContainer"><div><div id="chart-title">Graduation Rate</div><div class="chartbox"><canvas id="chart"></canvas></div></div></td></tr>' +
...
document
  .getElementById('subway_filter_btn')
  .addEventListener('click', subwayFilterPanel);

function subwayFilterPanel() {
  let element = document.getElementById('subway_filter_btn');
  element.classList.toggle('filter-button-active');
  let sub_arrow = document.getElementById('subway_arrow');
  // sub_arrow.setAttribute('style', 'fill: red');
  // console.log(sub_arrow);
  // document.getElementById("subway_panel").style.display = "block";
  let subway_panel = document.getElementById('subway_panel');
  if (subway_panel.style.display === 'none') {
    subway_panel.style.display = 'block';
  } else {
    subway_panel.style.display = 'none';
  }
}

document
  .getElementById('subway_reset')
  .addEventListener('click', resetSubwayFilterPanel);
function resetSubwayFilterPanel() {
  document.getElementById('subway_panel').style.display = 'none';
  document
    .getElementById('subway_filter_btn')
    .classList.toggle('filter-button');
}
...
function makeChart(schools) {
  ...
  var schoolColors = schools.map(function (d) {
    return d.DBN === '19K639' ? '#F15F36' : '#19A0AA';
  });
